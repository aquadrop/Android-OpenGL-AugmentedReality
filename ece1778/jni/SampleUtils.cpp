...
void SampleUtils::setIdentityM(float* matrix)
{
    for (int i = 0; i<16; i++)
        matrix[i] = 1.0f;
}
...
//left is matrix, right is vector
void SampleUtils::multipleMV(float* resultV, float* matrixA, float* vectorB)
{
    for (int i = 0; i<4; i++)
    {
        resultV[i] = 0.0f;
        for (int j = 0; j<4; j++)
            resultV[i] = resultV[i] + matrixA[i+j*4] * vectorB[j];
    }
}
