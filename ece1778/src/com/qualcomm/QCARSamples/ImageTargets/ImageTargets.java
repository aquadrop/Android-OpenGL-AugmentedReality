...
import com.android.constants.LookAt;
import com.android.ece1778.R;
import com.example.ece1778.VertexPackage;
...
public class ImageTargets extends Activity {
	...
	private static final int APPSTATUS_INIT_OBJECT = 1;
	private static final int APPSTATUS_INIT_QCAR = 2;
	private static final int APPSTATUS_INIT_TRACKER = 3;
	private static final int APPSTATUS_INIT_APP_AR = 4;
	private static final int APPSTATUS_LOAD_TRACKER = 5;
	...
	protected void onPause() {
		...
		destroyStoredVectorsNative();
		// QCAR-specific pause operation
		QCAR.onPause();
	}

	/** Native function to destroy the image vectors. */
	private native void destroyStoredVectorsNative();
	...
	private synchronized void updateApplicationStatus(int appStatus) {
		...
		case APPSTATUS_INIT_APP:
			...
			updateApplicationStatus(APPSTATUS_INIT_OBJECT);
			break;

		case APPSTATUS_INIT_OBJECT:
			// transfer the java vertices, normals and indices to C++ to
			// prepare for loading the object
			initObject();
	...
	/**
	 * Initialize the object vertices, normals, indices, lights to the C++ side.
	 */
	private void initObject() {

		// now extract the useful packages
		VertexPackage mVertexPackage = (VertexPackage) getIntent()
				.getSerializableExtra("VertexPackage");
		List<float[]> list_vertex_cor_tri = mVertexPackage
				.get_list_vertex_cor_tri();
		List<float[]> list_normal_cor_tri = mVertexPackage
				.get_list_normal_cor_tri();
		List<float[]> list_color = mVertexPackage.get_list_color();

		for (short i = 0; i < list_vertex_cor_tri.size(); i++) {
			initObjectNative(list_vertex_cor_tri.get(i),
					list_normal_cor_tri.get(i), list_color.get(i));
		}

		float[] mViewMatrix = new float[16];
		Matrix.setLookAtM(mViewMatrix, 0, LookAt.eyeX, LookAt.eyeY,
				LookAt.eyeZ, LookAt.lookX, LookAt.lookY, LookAt.lookZ,
				LookAt.upX, LookAt.upY, LookAt.upZ);

		loadViewMatrixNative(mViewMatrix);
	}

	/** C++ native function to load viewMatrix. */
	private native void loadViewMatrixNative(float[] viewMatrix);

	/** C++ native function for accepting vertices, normals */
	private native float initObjectNative(float[] vertex, float[] normal,
			float[] color);

	/** C++ native function for determining using sample or self-defined input. */
	private native float setHasInputNative(boolean has_input);

	/** Native function to test stored vectors. */
	private native float testStoredVectorsNative();
	...
	/** Native function for rotation. */
	private native void updateRotationNative(float xAngle, float yAngle);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			xAngle += dx * Constant.TOUCH_SCALE_FACTOR;
			yAngle += dy * Constant.TOUCH_SCALE_FACTOR;
			updateRotationNative(xAngle, yAngle);
			break;
		}
		xPrevious = x;
		yPrevious = y;
	...
	/** Native function to update scale factor. */
	public native void updateScaleFactorNative(float mscaleFactor);
	...
	private class ScaleListener extends
		...
		public boolean onScale(ScaleGestureDetector detector) {
			...
			mScaleFactor = Math.max(0.1f,
					Math.min(mScaleFactor, Constant.scale_max));
			updateScaleFactorNative(mScaleFactor);
			return true;
		}
	}
